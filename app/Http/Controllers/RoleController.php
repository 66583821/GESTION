...
class CentralController extends Controller
{
    ...
    public function traitement_login(Request $request)
    {
        ...
        $client2 = User::where('password', $request->input('password'))->first();
        ...
        if ($client) {
            if (Hash::check($request->input('password'), $client->password)) {
                if ($client->etat == 1) {
                    ...
                    if ($client->role == 'admini') {
                        $manags =  DB::select("select * from users where role = 'gerant'");
                        return view('vueadmin.liste_gerant', compact('manags'))->with('statut','admini');

                    } else {
                        if ($client->role == 'gerant') {
                            $attentes = DB::select('select * from users where etat = 0');
                            return view('vueadministr.liste_inscriptions', compact('attentes', 'cnt'));
                        } else {
